<template>
  <div class="card">
    <Tabs v-model:value="activeTab">
      ...
      <TabPanels>
        <TabPanel
          v-for="(kind, index) in resourceKinds"
          :key="kind"
          :value="index"
        >
          <ResourceTable
            :resources="resources[index]"
            :resourceHeader="$t(`resource_kind.${kind}`)"
            :icon="icons[kind]"
            @openNew="openAddEditModal(kind)"
            @editResource="openEditModal(kind, $event)"
